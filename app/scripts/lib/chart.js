    ...
    function main() {
      $.getJSON('/scripts/services/chart.json').done(function (dsn) {
        var chart = new Timely.Chart({
          element: $('#chart')[0],
          data: dsn['activities']
            .filter(function (d) { return d.user !== 'DSS'; })
            .sort(function (a, b) { return utc(a.start) - utc(b.start); }),
          rows: [
            _.extend(row('ant', 'user'), { title: 'By Antenna', height: 0.3 }),
            _.extend(row('user', 'ant'), { title: 'By Mission', })
          ]
        });
